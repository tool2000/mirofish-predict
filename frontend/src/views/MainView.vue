...
// Step State
const currentStep = ref(1) // 1: 그래프 구축, 2: 환경 구성, 3: 시뮬레이션 시작, 4: 보고서 생성, 5: 심화 상호작용
const stepNames = ['그래프 구축', '환경 구성', '시뮬레이션 시작', '보고서 생성', '심화 상호작용']
...
const handleNextStep = (params = {}) => {
  if (currentStep.value < 5) {
    ...
    // Step 2에서 Step 3으로 넘어갈 때 시뮬레이션 라운드 설정 기록
...
const handleGoBack = () => {
  if (currentStep.value > 1) {
    currentStep.value--
    addLog(`Step ${currentStep.value}로 돌아감: ${stepNames[currentStep.value - 1]}`)
  }
}
